...
```{bash, eval=FALSE}
cd /cbio/training/courses/2024/bacterial_wgs/data
# Wilcards
ls *.gz
ls *_R1_001.fastq.gz
```
...
Do each of the following tasks from your current directory using a single ls command for each:
 ...
 1. List all of the files in /cbio/training/courses/2024/bacterial_wgs/data that start with ‘101’.
 2. List all of the files in /cbio/training/courses/2024/bacterial_wgs/data that contain ‘6’.
...
chmod -w tesftfile-backup_R1.fastq
ls -l

# try removing file
rm tesftfile_R1-backup.fastq
...
## Searching files
```{bash, eval=FALSE}
grep "TTTTCTTTTT" toydata/tesftfile_R1.fastq
grep -B1 -A2 TTTTCTTTTT toydata/1tesftfile_R1.fastq
```

## EXERCISE
Search for the sequence GNATNACCACTTCC in the tesftfile_R1.fastq file. Have your search return all matching lines and the name (or identifier) for each sequence that contains a match.
