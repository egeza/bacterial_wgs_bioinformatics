...
grep -B1 -A2 NNNNNNNNNN testfile_R1.fastq > bad_reads.fastq
wc -l bad_reads.fastq
```

```{r search-in-many-files, eval=FALSE}
grep -B1 -A2 NNNNNNNNNN toydata/testfile_R2.fastq > bad_reads.fastq
wc -l bad_reads.fastq
```
...
grep -B1 -A2 NNNNN toydata/testfile_R1.fastq >> bad_reads.fastq
wc -l bad_reads.fastq

grep -B1 -A2 NNNNN toydata/testfile_R2.fastq >> bad_reads.fastq
wc -l bad_reads.fastq
```

The above commands can be combined to a single line code by using
```{r search-without-overwriting-one-line-code, eval=FALSE}
grep -B1 -A2 NNNNN toydata/*.fastq > bad_reads.fastq
wc -l bad_reads.fastq
```
...
grep -B1 -A2 NNNNN toydata/testfile_R1.fastq | less
grep -B1 -A2 NNNNN toydata/testfile_R1.fastq | wc -l 
# Check output matches expectation
grep -B1 -A2 NNNNN toydata/testfile_R1.fastq > bad_reads.txt
tail bad_reads.txt
# Fix the output by redirect to another grep
grep -B1 -A2 NNNNN toydata/testfile_R1.fastq | grep -v '^--' > bad_reads.fastq
